...
for i in range(10):
    try:
        ...
        logger.info("Connection MongoDB réussie en tant que root")
        break
    except errors.ServerSelectionTimeoutError:
        logger.warning("MongoDB pas encore pret, retry dans 2s.")
        time.sleep(2)
else:
...
if not any(u["user"] == DB_BOT_USER for u in existing_users.get("users", [])):
    ...
    logger.info(f"Le user pour '{DB_NAME}' '{DB_BOT_USER}' est créé.")
else:
    logger.info(f"Le user pour '{DB_NAME}' '{DB_BOT_USER}' existe déjà.")